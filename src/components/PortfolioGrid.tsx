...
// Asymmetric editorial grid pattern (col-span per index)
const PATTERN: { col: string }[] = [
  { col: 'lg:col-span-7' },
  { col: 'lg:col-span-5 lg:mt-10' },
  { col: 'lg:col-span-5' },
  { col: 'lg:col-span-7 lg:mt-12' },
  { col: 'lg:col-span-12' },
  { col: 'lg:col-span-6' },
  { col: 'lg:col-span-6 lg:mt-10' },
];
      ...
      <section className="pt-32 md:pt-40 pb-10 md:pb-14 bg-background">
        <div className="container-editorial">
          <Reveal className="max-w-3xl">
            ...
            <div className="mt-8 inline-flex items-center gap-3 rounded-full border border-foreground/10 bg-background/55 px-4 py-3 text-[0.62rem] uppercase tracking-[0.24em] text-foreground/52 backdrop-blur-xl">
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-foreground/10 bg-background/70">
                <ArrowRight size={12} />
              </span>
              <span>Curated project narratives with hover previews and layered motion</span>
            </div>
          </Reveal>
        </div>
      </section>
          ...
          {isLoading ? (
            <div className="grid grid-cols-12 gap-6 md:gap-10">
              {Array.from({ length: 4 }).map((_, i) => (
                <Skeleton key={i} className={`col-span-12 ${PATTERN[i].col} h-[26rem] rounded-[1.75rem]`} />
              ))}
            </div>
          ) : filtered.length === 0 ? (
